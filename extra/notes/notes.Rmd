...
* On loop exit, for each recorded assignment that points to a loop internal
  ...
  * Add an `ltar(..., N)` to the first use symbol.
  * Add an `lrec(..., N)` to the last assign expression; this probably needs to
...
* When hit a `ltar`, record the loop target id as well as the memory
...
* When hit a `lrec` look up the matching loop target and:
...
* When we hit `lvcopy`, go back and copy the `lrec` expression to the
  matching `ltar`.  For this to work we need to make sure the slot the `lrec`
...
Feels like there is a way to break this:
    ...
    for(i in x) {
      a <- ltar(b, 1)  // #2 <- a?
      e <- b           // #2 <- e   -- NOT okay b/c used after lrec
      b <- mean(y)     // #3 <- b
      d <- vcopy(c)    // #4 <- d
      b <- lrec(d, 1)
...
Produces something like:
    ...
    for(i in x) {
      a <- ltar(b, 1)  // #3 <- a?
      e <- b           // #3 <- e   -- NOT okay b/c used after lrec
      d <- vcopy(c)    // #4 <- d
      b <- lrec(d, 1)
      f <- e + 1       // #5 <- f
...
re-assignment?
    ...
    for(i in x) {
      a <- ltar(b, 1)  // #3 <- a?
      c <- a + 1
      a <- mean(x)     // memory of `ltar(b, 1)` potentially released?
      b <- x + y       // above memory used?

      // Or are we always safe because we only ever use `ltar` on a symbol

      e <- a
      a <- mean(x)     // this releases a? (name yes, but memory held by e)
      b <- lrec(d + 1, 1)       // this could write to old a memory?
      f <- e + 1       // #5 <- f
      copy_in_place(1) // copies contents of `d` to memory of `b`
    }
...
and somehow including that symbol in the loop body:
    ...
    for(i in x) {
      ...
      a <- ltar(b, .R2C_FOR_SYM_1)
      ...
      b <- lrec(d + 1, .R2C_FOR_SYM_1)
      ...
...
This seems okay.  `e` also should be updated same way.
    ...
    for(i in x) {
      ...
      c <- f
      ...
      b <- f <- lrec(d + 1, .R2C_FOR_SYM_1)
      ...
      copy_mem(.R2C_FOR_SYM_1)
    }

We will need to modify `copy_branch_dat` so that `loop_rec` and `loop_target`
don't interfere with assignment chains.  This is going to be a huge mess when
something needs to be both loop reconciled and branch reconciled.  But maybe not
too bad when we do the loop reconciliation first.
...
Might be a bit tricky to have a `brec` (branch rec) see through an `lrec`?
...
# Result Size Reconciliation

When we redirect something to the result,



# Nested Functions

Simplest implementation would be inlining with some kind of scope tracking for
symbols, but that disallows recursion.

Otherwise, it should be pretty straightforward to just generate multiple `run_x`
functions than the outer `run` calls.

Should probably try to implement tail recursion?

Do we allow full recursion?  Each level of recursion will require re-allocation,
which we likely cannot do ex-ante?  Is it the case that after the first level of
recursion we can classify the parameter types?  Probably not since it's possible
that through the recursion they change, not to mention there could be multiple
entry points to recursion within a single function in e.g. branches.
...
We need to deal with it because for non-numeric the coercion to numeric will
force a full copy every time.  Or is it every iteration?  Scenario is we have
`x` as an integer used in several different places.  In some places it is used
as an integer, and in others as a numeric?  Ok, that doesn't matter, we just
want to make sure we only turn `x` underlier into numeric once.  The tracked
type can stay as integer.

Need to think through:

* ctrl params: can evaluate to anything.
* flag params: can evaluate to true/false.
* external params: is there a restriction for these?  Obviously in `numeric(x)`
  we only allow scalar numerics/integers, but where does this get validated?  We
  could check for naked num on eval, and then each function needs to check it's
  requirements in C code (e.g. length for `numeric`).
* external symbols: can evaluate to naked numeric
...
See `extra/notes-control.Rmd`.

# Renaming

AFAIR most of what follows was written in the context of the store and re-use
optimization.
