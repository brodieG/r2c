...
## Constant vs. External

We've used the term external to mean constant, need to refine the semantics and
update the code to minimize chance for confusion:

* `external`: data outside of `data`, i.e. `MoreArgs` or unmatched.
* `internal`: data matching `data`?  Not sure.
* Constant:
  * Parameter.
  * Expression.
    * Implemented.
...
* `external` / `internal`: evaluation method.
* `constant` / `varying`: affected by varying data.
* `implemented` / `unimplemented`: whether `r2c` runnable or not.
...
`PAR.INT.LEAF` seems problematic?  Because it might require external evaluation.
So should probably be `PAR.LEAF`.  In generally, any of the `PAR.INT.*` terms
seem incorrect because any of those might be evaluated externally.

But we do also have the term "constant parameter" to reconcile with the above.
For those they always evaluate externally, so maybe PAR.EXT is okay, except that
PAR.INT can also use external evaluation?

So we start with PAR.CNST or PAR.VARY?  Not quite, we also have numeric vs
totally arbitrary.

I think the dimensions we want are:

* Eval: iternal/external_num/external_any
* Type: constant/varying

So preprocess should designate for each call whether it should internally or
externally evaluated, which it can do based on either the parameter type or
whether the call is supported or not.

Additionally alloc can check for constant/varying.  There seems to be misuse of
the leaf type.

## Derived Constant

Loops complicate matters.  Something might be a constant at the beginning of a
loop iteration, but due to rebinding later in the loop body it may no longer be
for the next one.

Even without dealing with the loop, the cuteness of being able to unmask
guarded symbols with a constant evaluation doesn't work as soon as we hit a
constant computed expression.  And we can't turn it into an eval'ed/cached
version because the C code was compiled with the call in sequence.

Need to track if all inputs into a function are constant, and if they are,
declare the function call to be constant too.

Does this mean we need to defer evaluation of constant expressions until we
reach a non constant container?  Otherwise in e.g.:

    mean(x + 1)

Where `x` is constant we'll compute `x`, `x+1`, and `mean(x+1)`.  So we need a
stack of constant expressions.  What about:

    sum(x + 1, x + 2)

Stack would start with `x + 1`, `x + 2`.

If input is external, defer evaluation, store the expression in a special stack,
and then the values in the argument `stack` object will have a flag that tells
them to look into that stack.  If it turns out that the call is call-variant
then at that point we can resolve those elements by the current method.

What happens with passive calls?  Probably okay?

How does this interfere with the caching process?  Do we look for pre-computed
sub-components?

What about branches?  Or things that got pre-processed into different
expressions (like branches).

What do we gain in doing this?
* Clarity of semantics (and thus documentation).
* Correct propagation.

We will never be able to treat as constant a call that involves an implemented
`r2c` function.  So in preprocess detect if a function is not supported.  If it
isn't then that entire tree should be marked as constant, which means we need to
reverse back the call data?

Super dicey, we might need to do a first pass to recognize non-supported
expressions and sub them out with special symbols.

## Parameter Validation

How do we deal with things like:

    if(i > 0) sum(seq_len(i))
    else sum(seq_len(abs(i)))

Constant param validation will prevent this from happening because validation is
done at allocation time.

We could add a constraint on branches so they don't depend on constant
parameters, but that's tricky because we've made invariance contagious.

Another approach is to move the validation into the function itself.  It adds a
branch that might otherwise be unnecessary, but it's going to be hard to avoid.
There are two points we need to think about.  At sizing, and then at runtime.
Can we avoid checking at sizing?  I guess apparently if the result size does not
affect anything.

One option would be to, if checks fail at sizing, return 0 size, and then error
at runtime.

This all seems fragile though; probably better to just document in branch
caveats.

# Guarded Symbols

We want an environment that we can add symbols to, but no one else can.  It
seems the only way to do this is to create an environment chain of locked
environments and bindings.

This might have performance implications for symbol
lookup, but probably not since unlikely binding chains will get huge, and for
data we can set everything in the same environment.  Could mitigate this by only
locking environments when we get to an `eval`.  Also we can potentially collapse
environments into new ones.

Right now main issue is we need to check all uses of `env.ext` and make sure
none are broken by the changes.  Biggest issue is likely `reconcile_env_ext`
with the use of stacks.  This will require careful thought to make sure we don't
mess something up, but it seems like it could be done by recreating the parent
environment instead of merging into an existing one.

For `reconcile_env_ext`, since we're processing depth first it should be the
case that at most we only ever have one set of branches outstanding, so so long
as we record the current `env.ext` when we hit the current TRUE branch, and save
then record the `env.ext` when we switch to the FALSE branch, we should be fine?
...
* Supported functions:
  * Generalities like numeric inputs (currently this is in internal/external).
  * Maybe an asterisk indicating semantic differences, expounded upon in a
    separate section?
  * Among other things also details which parameters are internal vs external,
    and maybe gives a quick example, then references the internal / external sub
    expression page.
  * Internal vs External:
    * Parameters.
    * Expressions vs Parameters (or is it really just external symbols)?
  * Semantic differences.

* Internal/external params.

* Performance
  * Memory is already its own page, so reference that?
  * Numeric vs non numeric.
  * Vectorized vs non vectorized, vs directly hand crafted C code.
  * Runner vs not runner.
...
Big problem with `rep.int` is that the times argument can be the same length as
the `x` argument, which only works with fixed size.  Can we leverage the new
`ftype` version?  Maybe, we have gmax, gmin, so we should be able to check for
the special case where they are equal and the same size as the second parameter.

# DLL count issue

We should be able to unload DLLs with `dyn.unload`.  Main problem referenced is:

> The function dyn.unload unlinks the DLL. Note that unloading a DLL and then
> re-loading a DLL of the same name may or may not work: on Solaris it used the
> first version loaded. Note also that some DLLs cannot be safely unloaded at
> all: unloading a DLL which implements C finalizers but does not unregister
> them on unload causes R to crash.

So it seems for our purposes this is fine so long as the ".so" is not preserved
in some way with intent to be reloaded.  Even then what would really have to
happen is for the user to manipulate the C code and generate an so file with the
same name.

If we can register a finalizer to the r2c object such that when it goes out of
scope it unloads the DLL that probably will work reasonably well.

Finalizer seems to works, but it's obvious that `gc(full=TRUE)` does not
actually `gc` all things that it should, so adding the finalizer only slows down
the point at which we eventually run out of DLLs.

And in the case of the unitizer tests it's not clear that it's actually reducing
the number of DLLs, and I can't seem to get the DLLs to unload, or more
specifically the r2c environments to GC.

No great solution for this.  For our purposes we'll just add a method of
recording registered DLLs and dumping them as part of the unitizer tests so the
tests themselves don't cause a problem.
...
## Result Size vs Intermediate Size

Final result must get it's own 

## Reconcile asize vs size

For results can we detect that we're dealing with the last computing call and
disallow re-use for non-matching size?  And thus don't have to worry about the
alternative?  Yes, we just need to safely detect when we're at the last
computing call.  But can we do that easily with branches and passive calls?

We should be able to travel through the list of calls and find the last
non-passive call by only storing a new call id if it is non passive.  If it is
a branch though? Might be okay so long as the branch reconciliation obeys the
rules.

Reconcile's are guaranteed to be loop local, but not necessarily vcopy's.  Can
we rely on size being good.
