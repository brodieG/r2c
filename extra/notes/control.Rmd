...
The stack / alloc numbers aren't exactly right.  We need to check that:

* When we update `call.dat` in `lrec` handling, do we also need to update the
  stack?
* We see:

    language r2c::lset(x = w, rec.i = 1L)
    chr "lset"
    int [1:3] 17 18 18

  But we kind of would like it to be `17 18 17`; do we have any control of what
  the passive's return as far as alloc ids?  Is it the case that so far for all
  the passives it's the second argument that gets returned?  Is the easiest
  solution to change the order of the lset arguments?
