...
* What happens with `by` == `width` for `window_i_exec`.  It will cause values
  on the boundary to be included in two adjacent windows.  Is that bad?  It's
  the opposite of `window_exec` where they are not.  If we change it to be
  closed(?) on the right though then what is the meaning of `align='right'`?
  Right now we define that as `width`.  Do we allow ourselves to say that with
  `align='right'` the base index is not actually included in the window?
...
It does, it just gets weird if e.g. `width = 0.1`.  The alternative is to define
"right" as `align = width`.  Then the two methods are consistent, albeit
inconsistent with `rollapply`.

Is this consistent with `align = "center"` equal to `align = width / 2`?  It can
be if we declare that for even sized windows it is the leading part of the
window that gets the extra element, as opposed to the trailing one.  If we had
intervals closed on the right instead of the left then it would be consistent,
but that goes against the logic of the trailing intervals and midnight being
part of the day that follows it.

    w_by(
      f, x, w,
      i=seq_along(x), by=1, align='center',
      start=i[1], end=i[length(i)], partial
    )
    w_at(
      f, x, w,
