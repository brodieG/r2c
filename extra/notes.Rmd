...
Need to track size of all variables in every branch and before the if, and check
that they all resolve to the same size after the if, at least for those that get
used after the if (i.e. intermediate variables not used again don't matter).

We'll need to allocate for every branch, even if the taken branch at run time
might not set/change a variable.

What about variables created in loops branches are not taken and then attempted
to be used later?

# Rename stack

List, with for names the original names, and for values all the things the names
were renamed to, and context for each of those (e.g. pre-loop, loop depth,
condition n, condition depth).
