...
The idea of dropping the braces was cute, but it wreaks havoc with all sorts of
other assumptions we make about depth.

Right now the only way we get it to work is by having a C implementation of
braces that copies from source to result.  Could it work if instead we managed
to get braces to designate the same read and write?  It's a bit tricky because
of the possibility of a result.

So if we add a `self` option to `alloc_dat`, should work probably, but what if
that self is e.g. protected?  Does it matter?  Is all the what need to do update
the depth then?  Seems right.  Assignment should probably do something similar.

Then, we restore the brackets code, but it truly becomes a no-op.

Main challenge right now is that we need to convey the special case where the
result needs to be written to the final result and not just an intermediate
point, imagine:
...
This is going to be several layers deep, but still needs `b + a` written to the
result, preferably without having to copy it from unnecessary temporary
assignments.

We could have braces promote the result to a new depth.  Assignments don't
strictly need to do that because they add the protection.

But ultimately the problem is we need to know in the above that `b + a` should
be writing to the result.  We would know that by having analyzed the linearized
call and finding the last non braces.

# Assignments

Need to restructure to allow multiple symbols to point to the same memory
allocation.

When a memory allocation gets a name, it gets a protection.  For each
allocation, we need to track all the names bound to it, and the protection each
of those conveys.

So need two lists, one tracking for each data, what symbols are bound to it, and
another, for each symbol, what the latest protection it provides.  We also need
a lookup table for each symbol, what data it points to, that will respect
masking, etc.

We really need `append_dat` and `alloc_dat` to have some structural guarantees
so we don't accidentally mess it up.

Also, `append_dat` can add names.

Some names can be overwritten, but others can't (e.g. data, later when we have
function scope, outer function ones).

For lookups, probably we just use a named vector with possibly repeated names,
and some symbol assignment level (this will be the scope for e.g. future
function environments, and for data today).

To determine whether a slot is free, it needs to check that it is not in the
name vector.

Name vector is cleaned up anytime there is an assignment, and after every
iteration (protection expiration).

Each name needs:
* A scope (cannot overwrite something of lower scope).
* A data id it points to.
* An order in the name list.

Each iteration needs cleanup:
* Remove any out of scope names.
* Remove any expired names.
