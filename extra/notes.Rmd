...
Figure out how to report missing arguments in `alloc`
...
* Figure out why external vectors are being duplicated.  Were seeing:
  ...
  for the slope calculation with y as 1:3 (literal).
...
* How can we efficiently warn of "longer object length is not a multiple" in
  ...
  time, annoyingly.
* If we use environments to carry `data` and `MoreArgs` around, we should make
  sure we don't introduce reference issues / reference count issues.
* The check might be better done with an explicit check function.
...
* What is the implementation cost of allowing internal functions to advance the
  ...
  iteration?
* Implement sum64 and mean64 to use double accumulator.
...
(looking at timings at group size of 1000).
