...
* Make sure to test the `known + group` size issue with varying group sizes.

* Document that `br.hide` business.
...
* A bit weird that we need to propagate the `rec` value in alloc?  Maybe because
  of allocation re-use?  Actually we might be able to drop this now that we go
  back to looking for `rec(...)` for the return value (i.e undo 3b053fd).
...
* Need a way to get the clean call to show in errors, e.g. this is terrible, but
  hard to fix because we need the `if_test` part of it:

    Error in alloc(x = preproc, data = do, gmax = gmax, gmin = gmin, par.env = enclos,  :
      Potentially unequal sizes for parameters true, false in a function that requires them to be equal sized:
    r2c::r2c_if(true = r2c::if_true(x <- r2c::rec(r2c::vcopy(y))),
        false = r2c::if_false({
            x <- r2c::rec(r2c::vcopy(x))
            numeric(0)
        }))

* We now track `rec` status both in the "names" matrix as well as in the `alloc`
  data.  Should we drop everything in that matrix (and maybe the stack) that can
  be recovered from `alloc` via `id0` to mitigate potential synchronization
  issues.
...
* Could reconciliation use a previously allocated but freed vector?  Need to
  make absolutely sure it e.g. is not used in the TRUE branch, so maybe not?
  Feels a bit dangerous.

* Check into why NA size is not carrying forward across calculations and instead
  getting replaces by `gmax`.

* Does re-use correctly recognize that an assignment in one branch cannot be
  used in the alternate (particularly if the assignment happens in the TRUE
  branch?

* if/else with NA test.

* Check that return value actually has a `rec`.

* Does allocation recognized nested reconciliation?

* Symbol expiration logic in `alloc` might not work well with branches.

* Make sure that changing `r2c_if` to non-passive didn't mes something up.

* Check that reconciliation is not attempting to reconcile non-branch-local
  memory.  Only in-branch allocations are reconcilable, i.e. don't reconcile
  these: `x <- mean(z); y <- mean(w); if(a) x else y` without additional
  `vcopy`.

* Implement `r2c::ccopy` to copy constants into vectors.
* Currently `if/else` with missing `else` use `numeric(0)` as a literal value
  rather than language.  Need some mechanism for handling this, maybe actual
  return a special symbol that can be bound to an empty numeric as part of the
  early data?  This also relates to the general issue with not handling
  literals.

* `clean_call` should have additional levels to remove `vcopy` / `rec`.
