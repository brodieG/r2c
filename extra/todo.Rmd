...
* Check that a non-local symbol inside an active expression doesn't get copied
  ...
  last (e.g. `mean` doesn't because of `na.rm`, etc.).
...
* Get rid of `data[['passive']]` if we end up not needing to auto-promote `last`
  case in `first.assign` branch.

* `copy_branchdat` still not correct because the final return value may also
  need to be copied.

* Local symbols are completely cleared by nested branches, but we only want to
  clear those symbols that are re-assigned.
