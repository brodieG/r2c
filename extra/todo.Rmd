...
* Transform should be an optimization?
* Fix over-optimization where expression that is already assigned to a symbol
  gets assigned to a sub-symbol.  This is not trivial because 
...
* Symbol resolution apparently skips non-naked data; does that make sense or
  should it find it and error?
* What happens if we have an assignment at depth 0?
...
* `r2cq(test)` produces weird error (note that's just a symbol).

* Are we okay with doing the call substitution before match-calling?  That's
  going to change param evaluation order.  Probably not okay.

* Make sure vecrec works with `...`, e.g. as would be needed for `paste(...)`
  (although maybe that's not the best example given 0-len behavior).
...
* Should we add `rm` statements (optionally) for `reuse_calls` use in R to
  reduce peak memory usage?
* What do indices do we want to record in a re-used call?
