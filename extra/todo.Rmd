...
* Think carefully about the following case.  Can we get away without a
  reconciliation (as it is it's just not right - mismatched rec):
...
* Rationalize documentation of "implementation detail" functions like `r2c_if`
  ...
  add a family that connects them. Make sure to explain how we generate the C
  code for control structures as that is not at all obvious.
* Document for:
