...
* Note that groups are pre-sorted.
* Show example of e.g. `rolli_exec`.
* Cleanup the benchmark code for group and windows, and post links to them.
* Avoid repetition from the iteration section and related works sections.
* Remove benchmarks from `roll*_exec` docs.

## General

* Release fixed `vetr` version!
...
* Can we have zero length data.  Both zero length vectors, and empty `data`
  list?  We should be able to.  I think we just need to test zero length data.
...
* Fix README links so they all work e.g. raw github URLs, and also whether to
  use vignettes or what.
...
* Look into whether we can efficiently iterate over an altrep without expanding
  ...
  e.g. iterate by region which would generate and expand small chunks.
