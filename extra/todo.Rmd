...
* Why do we loose a local assignment from `data[[B.NAMED]]` from branch rec?

* Rename `rec` `brec` to distinguish more clearly from `lrec`?

* It seems like we don't need `luse` as in `lrec(.R2C.FOR.SYM.1)` we retain the
  alloc id of the original used symbol memory which should be the same as
  `.R2C.FOR.SYM.1` as we aliased it.  Maybe this is less important to fix now
  that we've rationalized logic a little with `lrec_update`?  Actually, we also
  have logic in preproc, etc, so really we should drop this.
...
* It would be nice to be able to assume there are no nested braces ever in
  ...
  anyplace that isn't robust to nested braces tests for them.
...
* Need to make sure that alloc doesn't release `for` bindings until end of loop.
  ...
  there is we're using the original loop names, not the rebuilt ones.
  Need to add a test that confirms this works.
...
* What happens when we subassign in a branch?  This should be handled?
...
* Should we just used nested environments for name bindings, where each name
  ...
  (great for looking up names though).
