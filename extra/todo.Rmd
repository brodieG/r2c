...
* Strictly it's iterated calculations, not iterated statistics, but it doesn't
  make a whole lot of sense to use r2c without stats as you probably could do
  just fine with regular R (but what about e.g. reversing by group - could
  reverse whole thing, and then order by group
...
* Nested functions?  I.e. recurse into un-implemented functions, and if they are
  ...
  possible, except the tail variety (and then that will require extra thought).
...
* Better introspection tools for intermediate products so we can see what they
  are.
* Are names sufficient to disqualify from nakedness (no)?  How do names behave
  in recycling (dropped now)?  Do we want to allow other attributes to persist?
