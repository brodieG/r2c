...
#' environment of the function.  Currently there is no way to change the
#' function environment (`environment(r2c_fun) <- x` will likely just break the
#' function).
#'
...
clean_call <- function(x) {
  if(is.call_w_args(x)) {
    ...
    if(!is.null(fun.defn) && !is.null(names(x))) {
      ...
      default.args <- vapply(defn.frm, function(x) !identical(x, MISSING), TRUE)
