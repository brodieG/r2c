...
# time by remapping the different allocations in each branch to a new shared
# allocation.  Subsequent accesses will then read from the shared location.  For
# this to work, prior to re-mapping the allocations bound to each symbol across
# the branches must be branch-local.  The same remapping happens with the branch
# return value.  Substantial complexity is added by the possibility of an
# assignment also being a return value, e.g. `if(a) x <- y`.
#
# Consider:
#
#     if(a) mean(x)
...
# take the two allocations the allocator will have generated for each
# calculution, and remap them to a single new allocation.  If the allocations
# are not the same size that will result in an allocation-time error.
#
#     if(a) mean(x)
#     else y
#
# `y` is not branch local memory anymore, so we cannot just remap it freely
# without potentially intefering with branch-external calculations.  In some
# cases we _could_, but discerning the cases where it is safe to do so requires
# substantial additional complexity.  So to resolve the above the expression
# would be processed into:
#
#     if(a) mean(x)
#     else vcopy(y)
#
# Here is an example where failing to copy even an `r2c` local allocation (but
# not branch local) would cause problems:
#
  ...
  if (is.symbol(x)) {
    ...
    if(in.branch) {
      ...
      if(length(assign.to)) {
        if(sym.local.cmp) {
          ...
          data <- add_candidate_callptr(
            data, index, triggers=assign.to, copy=TRUE, rec=TRUE
          )
        }
      }
    ...
    if(branch.res) {
      ...
      if(!call.passive && !length(assign.to)) {
        ...
        } else if (call.assign || !call.passive) {
          # Always make candidates for multi-assignments
          data <- add_candidate_callptr(
            data, triggers=trigger, index, rec=TRUE, copy=call.passive
          )
        }
      }
    } else if (in.branch) {
      if(call.assign && !first.assign) {
        ...
        data <- add_candidate_callptr(
          data, index, triggers=trigger, rec=TRUE, copy=FALSE
        )
      }
    }
  }
