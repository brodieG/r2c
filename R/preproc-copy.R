...
# This requires copying the value assigned during the final set to the memory
# location of the initial symbol use.  This does not happen automatically
# because r2c keeps allocating new slots for each subsequent calculation, so if
# we rebind a value to an existing symbol, it is not written to the original
# location (and can't be because that original location might be aliased to a
# different symbol).  This code ensures that the original location is un-aliased
# by adding a `vcopy` before loop start, and then adds an explicit copy from the
# final location to that one via `lrec` at the end of the loop.  `lrec`
# implicitly retains the original location by referencing a special alias in
# form `.R2C.FOR.SYM.N`.
#
# `alloc` can then look for `lset` and `lrec` pairs matched up by the unique
# "rec.id" values generated by this function.
#
# @param x call tree
